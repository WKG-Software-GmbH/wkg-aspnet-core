...
internal class HmacSha256SigningFunction(HmacOptions options) : IJwtSigningFunction
{
    public string Name => "HS256";

    public int SignatureLengthInBytes => HMACSHA256.HashSizeInBytes;

    public int Sign(ISessionKey sessionKey, ReadOnlySpan<byte> buffer, Span<byte> signature)
    {
        ArgumentOutOfRangeException.ThrowIfLessThan(signature.Length, HMACSHA256.HashSizeInBytes, nameof(signature));
        ...
        int bytesWritten = HMACSHA256.HashData(keyBufferSpan, buffer, signature);
    ...
    public int Sign(ISessionKey sessionKey, Stream stream, Span<byte> signature)
    {
        ...
        sessionKey.WriteKey(keyBufferSpan[^sessionKey.Size..]);
        // compute Hash-based Message Authentication Code (HMAC) using server secret and session key
        int bytesWritten = HMACSHA256.HashData(keyBufferSpan, stream, signature);
        ArrayPool.Return(keyBuffer, clearArray: true);
        return bytesWritten;
    }
}
